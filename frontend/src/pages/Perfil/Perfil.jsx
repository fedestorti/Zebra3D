...
export default function Perfil() {
  ...
  try {
    ...
    if (isFollowing) {
      await API.delete(`/usuarios/${apodo}/seguidores`, config);
      setIsFollowing(false);
      setFollowers(c => c - 1);
    } else {
      await API.post(`/usuarios/${apodo}/seguidores`, {}, config);
      setIsFollowing(true);
      setFollowers(c => c + 1);
    }
  } catch (err) {
...
const handleRate = async value => {
 if (value < 1) value = 1; 
  ...
  try {
    ...
    await API.post(`/usuarios/${apodo}/resenas`, { calificacion: ratingValue }, config);
  ...
  >
    {isFollowing ? 'Siguiendo' : 'Seguir'}
            ...
            {/* Rating con react-simple-star-rating */}
